...
solutionMatrices = [ (heuristic(initial,goal),[[2,0,3],[1,8,4],[7,6,5]]) ]
parentQueue = [ initial ]
finalAnswer =  [ ]
for parent in solutionMatrices:
    ...
    if indexC != 2:
        ...
        temp = (heuristic(right, goal),right)
        if right == goal:
            finalAnswer = right
            solutionMatrices.append(temp)
            parentQueue.append(parent[1])
